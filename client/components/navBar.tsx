...
export default function Navbar() {
  ...
  const navItems: NavItem[] = [
    ...
    { label: 'Dashboard', icon: LayoutDashboard, href: '/dashboard', onClick: null },
    { label: 'Profile', icon: User, href: '/profile', onClick: null },
    ...(user
      ? isAdmin
        ? [
            { label: 'Admin', icon: MessageCircle, href: '/admin', onClick: null },
            { label: 'Balances', icon: Wallet, href: '/admin/balance', onClick: null },
          ]
        : [{ label: 'Support', icon: MessageCircle, href: '/chat', onClick: null }]
      : []),
    user
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute top-full left-0 w-full bg-black flex flex-col gap-2 p-4 md:hidden border-t border-white/10"
          >
            {navItems.map((item) => (
