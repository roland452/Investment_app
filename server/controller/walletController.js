...
// to verify server-side before crediting (never trust the frontend alone)
  ...
  try {
    ...
    try {
      ...
      if (txResult.rows.length === 0) {
        ...
        return res.status(409).json({ error: 'Transaction already processed or not found' });
      }
...
// Step 3 (backup): Flutterwave webhook, in case the user closes the app
// before the frontend can call verifyTopup
exports.webhook = async (req, res) => {
  ...
  if (event.event === 'charge.completed' && event.data.status === 'successful') {
    ...
    try {
      await client.query('BEGIN');
      const txResult = await client.query(
        "UPDATE transactions SET status='success' WHERE reference=$1 AND status='pending' RETURNING user_id",
        [reference]
      );
      if (txResult.rows.length > 0) {
